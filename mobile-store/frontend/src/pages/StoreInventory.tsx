...
export default function StoreInventory() {
  ...
  const fetchInventory = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get('http://localhost:8000/api/inventory/store', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const fetchTransfers = async () => {
    try {
      const res = await axios.get('http://localhost:8000/api/transfers/', { headers: { Authorization: `Bearer ${token}` } });
