...
export default function ReceiveParts() {
    ...
    const fetchInitialData = async () => {
        try {
            const [supRes, modRes, invRes] = await Promise.all([
                axios.get('http://localhost:8000/api/parts/suppliers', { headers: { Authorization: `Bearer ${token}` } }),
                axios.get('http://localhost:8000/api/models/', { headers: { Authorization: `Bearer ${token}` } }),
                axios.get('http://localhost:8000/api/parts/', { headers: { Authorization: `Bearer ${token}` } })
            ]);
    ...
    const handleReceive = async (e: React.FormEvent) => {
        ...
        try {
            await axios.post('http://localhost:8000/api/parts/receive', {
                ...formData,
