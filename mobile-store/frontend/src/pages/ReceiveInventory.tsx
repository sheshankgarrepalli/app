...
export default function ReceiveInventory() {
    ...
    const handleExecute = async () => {
        ...
        try {
            const res = await axios.post('http://localhost:8000/api/transfers/bulk-receive', {
                imeis: scannedImeis,
                notes
            }, {
    ...
    return (
        <div className="flex flex-col h-full bg-zinc-50">
            <header className="p-6 bg-white border-b border-zinc-200 flex justify-between items-center">
                <div>
                    <h1 className="text-lg font-bold text-zinc-900">Asset Intake</h1>
                    <p className="text-xs text-zinc-500 mt-1">Chain of custody & inventory synchronization</p>
                </div>
            </header>
            ...
            <div className="flex-1 grid grid-cols-12 overflow-hidden">
                ...
                <div className="col-span-4 bg-white border-r border-zinc-200 p-6 space-y-8 overflow-y-auto">
                    <form onSubmit={handleScan} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-semibold uppercase tracking-[0.1em] text-zinc-500">Intake Notes</label>
                            <textarea
                                value={notes}
                    ...
                    {summary && (
                        <div className="p-6 bg-zinc-50 border border-zinc-200 rounded-lg space-y-4 animate-in fade-in slide-in-from-bottom-4 shadow-sm">
                            <div className="flex justify-between items-center">
                                <span className="text-xs font-semibold uppercase tracking-widest text-zinc-400">Receipt Summary</span>
                                <button onClick={() => setSummary(null)} className="text-zinc-400 hover:text-zinc-900 transition-colors"><X size={16} /></button>
                            </div>
                            <div className="text-3xl font-bold text-zinc-900">{summary.success_count} <span className="text-xs text-zinc-400 uppercase tracking-widest">Success</span></div>
                ...
                <div className="col-span-8 flex flex-col bg-zinc-50 overflow-hidden">
                    <div className="p-4 border-b border-zinc-200 bg-white flex justify-between items-center">
                        <label className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">Intake Batch ({scannedImeis.length})</label>
                                        ...
                                        {scannedImeis.map((imei, index) => (
                                            <tr key={imei} className="border-b border-zinc-100 hover:bg-zinc-50/50 transition-colors group animate-in fade-in slide-in-from-top-2">
                                                ...
                                                <td className="px-8 py-4 text-right">
                                                    <button onClick={() => removeImei(imei)} className="text-zinc-300 hover:text-rose-600 transition-colors">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </td>
                                            </tr>
