...
export default function POS() {
  ...
  const handleRetailCheckout = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.post('http://localhost:8000/api/pos/invoice', payload, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleB2BCheckout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:8000/api/pos/bulk-checkout', payload, { headers: { Authorization: `Bearer ${token}` } });
