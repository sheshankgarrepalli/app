...
def export_report(
    ...
    if report_type == "inventory":
        ...
        q = db.query(
            models.Invoice.id, models.Invoice.invoice_number, models.Invoice.created_at,
        ...
        for r in q.order_by(models.Invoice.created_at.desc()).all():
            cust = r[3] or f"{r[4] or ''} {r[5] or ''}".strip() or "Walk-in"
            total = r[8] or 0
            ...
            balance = total - paid
            writer.writerow([
                r[1], r[2].isoformat() if r[2] else "", cust,
                round(r[6] or 0, 2), round(r[7] or 0, 2), round(total, 2),
