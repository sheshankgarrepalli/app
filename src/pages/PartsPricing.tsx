...
export default function PartsPricing() {
    ...
    const fetchUnpriced = async () => {
        try {
            const res = await axios.get((import.meta.env.VITE_API_URL ?? 'http://localhost:8000') + '/api/parts/unpriced', {
    ...
    const handlePriceUpdate = async (intakeId: number) => {
        ...
        try {
            await axios.put((import.meta.env.VITE_API_URL ?? 'http://localhost:8000') + '/api/parts/price', {
                intake_id: intakeId,
    ...
    return (
        <div className="flex flex-col h-full bg-zinc-50">
            <header className="p-6 bg-white border-b border-zinc-200 flex justify-between items-center">
                ...
                <button
                    onClick={() => axios.post((import.meta.env.VITE_API_URL ?? 'http://localhost:8000') + '/api/parts/seed', {}, { headers: { Authorization: `Bearer ${token}` } }).then(() => { alert("Mock Data Seeded"); fetchUnpriced(); })}
