...
export default function UserManagement() {
    ...
    return (
        <div className="p-8 max-w-6xl mx-auto space-y-12">
            <header>
                <h1 className="text-xl font-bold text-zinc-900 mb-1">Identity & Access</h1>
                <p className="text-xs text-zinc-500 mt-1">Manage employee credentials and store assignments</p>
            </header>

            <div className="grid grid-cols-12 gap-12">
                {/* CREATE USER FORM */}
                <div className="col-span-12 lg:col-span-4">
                    <form onSubmit={handleCreateUser} className="space-y-6 border border-zinc-200 p-8 bg-white rounded-lg shadow-sm">
                        <div className="flex items-center gap-2 mb-4">
                            <UserPlus size={18} className="text-zinc-400" />
                            <h2 className="text-xs font-semibold uppercase tracking-widest text-zinc-500">Provision New User</h2>
                        </div>

                        <div className="space-y-4">
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400 ml-1">Email Address</label>
                                <div className="relative group">
                                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-300 group-focus-within:text-zinc-900 transition-colors" size={14} />
                                    <input type="email" value={email} onChange={e => setEmail(e.target.value)} required className="input-stark w-full pl-10 py-3" />
                                </div>
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400 ml-1">Temporary Password</label>
                                <div className="relative group">
                                    <Key className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-300 group-focus-within:text-zinc-900 transition-colors" size={14} />
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400 ml-1">System Role</label>
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400 ml-1">Assignment</label>
                            ...
                            <tbody className="text-sm">
                                {users.map(u => (
                                    <tr key={u.id} className="border-b border-zinc-100 hover:bg-zinc-50/50 transition-colors">
                                        <td className="px-8 py-5 text-zinc-900 font-semibold">{u.email}</td>
                                        <td className="px-8 py-5">
                                            <span className="flex items-center gap-2 text-zinc-500 uppercase tracking-widest text-[10px] font-bold">
