...
export default function InvoicesDashboard() {
    ...
    const fetchInvoices = async () => {
        setIsLoading(true);
        try {
            const res = await axios.get((import.meta.env.VITE_API_URL ?? 'http://localhost:8000') + '/api/pos/invoices', {
        ...
        <div className="flex flex-col h-full bg-zinc-50 dark:bg-[#0a0a0b]">
            <header className="p-6 bg-white dark:bg-[#141416] border-b border-zinc-200 dark:border-[#1f1f21] space-y-6">
                <div className="flex justify-between items-end">
                    <div>
                        <h1 className="text-lg font-bold text-zinc-900 dark:text-[#e4e4e7]">Financial Ledger</h1>
                        <p className="text-xs text-zinc-500 dark:text-[#71717a] mt-1">Invoice management & revenue tracking</p>
                    </div>
                    <div className="flex gap-3">
                        <div className="relative">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 dark:text-[#a1a1aa]" size={14} />
                            <input
                                type="text"
                ...
                <div className="bg-white dark:bg-[#141416] border border-zinc-200 dark:border-[#1f1f21] rounded-lg shadow-sm overflow-hidden">
                    <table className="w-full text-left border-collapse table-fixed">
                        ...
                        <tbody className="text-sm">
                            {isLoading ? (
                                <tr><td colSpan={6} className="py-32 text-center text-zinc-400 dark:text-[#a1a1aa] animate-pulse font-medium">Loading Ledger...</td></tr>
                            ) : filteredInvoices.map(inv => (
                                <tr key={inv.id} className="border-b border-zinc-100 dark:border-[#1a1a1c] hover:bg-zinc-50 dark:hover:bg-[#1a1a1c] dark:bg-[#0a0a0b]/50 transition-colors group">
                                    <td className="px-6 py-4">
                                        <div className="text-zinc-900 dark:text-[#e4e4e7] font-bold uppercase tracking-widest text-xs">{inv.invoice_number}</div>
                                        <div className="text-[10px] text-zinc-400 dark:text-[#a1a1aa] font-semibold tracking-widest mt-0.5 uppercase">{inv.store_id}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-zinc-700 dark:text-[#e4e4e7] font-bold uppercase tracking-tight truncate text-xs">
                                            {inv.customer?.company_name || inv.customer?.name || 'Walk-in Customer'}
                                        </div>
                                        <div className="text-[10px] text-zinc-400 dark:text-[#a1a1aa] font-semibold tracking-widest mt-0.5 uppercase">{inv.customer?.crm_id || 'N/A'}</div>
                                    </td>
                                    <td className="px-6 py-4 text-zinc-500 dark:text-[#71717a] font-medium">
                                        {new Date(inv.created_at).toLocaleDateString()}
                                    </td>
                                    <td className="px-6 py-4 text-zinc-900 dark:text-[#e4e4e7] font-bold">
