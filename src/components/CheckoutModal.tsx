...
export default function CheckoutModal({ isOpen, onClose, totalAmount, onComplete, isProcessing }: CheckoutModalProps) {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-2xl border border-zinc-200 shadow-2xl animate-in fade-in zoom-in duration-200 rounded-xl overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-zinc-100 flex justify-between items-center bg-zinc-50">
                    <div>
                        <h2 className="text-xl font-bold text-zinc-900">Checkout / Tender</h2>
                        <p className="text-xs text-zinc-500 mt-1">Split payments and finalize transaction</p>
                    </div>
                    <button onClick={onClose} className="p-2 text-zinc-400 hover:text-zinc-900 transition-colors bg-white rounded-full border border-zinc-200 shadow-sm">
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="p-6 overflow-y-auto flex-1">
                    <div className="flex justify-between items-end mb-6">
                        <div>
                            <div className="text-xs font-semibold uppercase tracking-[0.1em] text-zinc-500 mb-1">Amount Due</div>
                            <div className="text-4xl font-bold text-zinc-900">${totalAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                        </div>
                        <div className="text-right">
                            <div className="text-xs font-semibold uppercase tracking-[0.1em] text-zinc-500 mb-1">Remaining Balance</div>
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center border-b border-zinc-100 pb-2">
                            <h3 className="text-sm font-semibold text-zinc-700">Payment Methods</h3>
                            <button type="button" onClick={handleAddPayment} className="text-xs font-semibold uppercase tracking-widest text-zinc-900 flex items-center gap-1 hover:text-zinc-600 transition-colors">
                        ...
                        <div className="space-y-3">
                            {payments.map((p) => (
                                <div key={p.id} className="flex items-center gap-3 bg-zinc-50 p-3 rounded-lg border border-zinc-100 animate-in slide-in-from-left-2">
                                    <div className="w-1/3">
                                        <select 
                                            value={p.method}
                                            onChange={(e) => handleUpdatePayment(p.id, 'method', e.target.value)}
                                            className="w-full bg-white border border-zinc-200 rounded-md py-2 px-3 text-sm font-medium focus:border-zinc-900 outline-none"
                                        >
                                    ...
                                    <div className="flex-1 relative">
                                        ...
                                        <input 
                                            ...
                                            className="w-full pl-8 pr-3 py-2 bg-white border border-zinc-200 rounded-md text-sm font-bold text-right focus:border-zinc-900 outline-none"
                                        />
                                    </div>
